...
struct MergedActivityIndicator: View {
    ...
    private var baseCenterX: CGFloat {
        ...
        let positions = activities.compactMap { activity -> CGFloat? in
            guard let time = activity.scheduledTimeFor(date: selectedDate) else { return nil }
    ...
    private var splitActivityXPosition: CGFloat {
        ...
        return max(
            fixedItemSize / 2,
            min(containerStartX + baseOffset + splitDragOffset, width - fixedItemSize / 2))
    }
    ...
    var body: some View {
            ...
            if !isSplitting {
                ...
                if isGroupDragging, let targetTime = dragTargetTime {
                    timeBadgeView(
                        ...
                        color: Color(hex: activities.first?.colorHex ?? "#CCCCCC")
                    )
        ...
        .onChange(of: baseCenterX) { _, _ in
            resetState()
        }
    }
    ...
    private var mergedContainerView: some View {
        HStack(spacing: 0) {
            ForEach(Array(activities.enumerated()), id: \.element.id) { index, activity in
                activityItemView(
                    activity: activity, index: index, isLast: index == activities.count - 1)
    ...
    private var remainingContainerView: some View {
        ...
        return Group {
            if remainingActivities.count == 1, let activity = remainingActivities.first {
                ...
                // Multiple remaining
                HStack(spacing: 0) {
                    ForEach(Array(remainingActivities.enumerated()), id: \.element.id) {
                        index, activity in
                        activityItemView(
                            activity: activity, index: index,
                            isLast: index == remainingActivities.count - 1)
        ...
        DragGesture(minimumDistance: 3)
            .onChanged { value in
                if !isDragging {
                    ...
                    let grabXRelativeToContainer =
                        value.startLocation.x - hitAreaCenterX + containerWidth / 2
                ...
                case .left, .right:
                    ...
                    dragTargetTime = TimelinePositionHelper.timeFromPosition(
                        max(0, min(newX, width)), width: width)
            ...
            .onEnded { value in
                        ...
                        let firstTime = firstActivity.scheduledTime
                    {
                        ...
                        for activity in activities {
                            if let activityTime = activity.scheduledTime {
                                ...
                                let actMinute = Calendar.current.component(
                                    .minute, from: activityTime)
                ...
                case .left, .right:
                    ...
                    if let activity = splittingActivity {
                        ...
                        let newTime = TimelinePositionHelper.timeFromPosition(
                            max(0, min(finalX, width)), width: width)
...
enum ActivityGroupingHelper {
    ...
    static func groupActivities(
        _ activities: [Activity], width: CGFloat, selectedDate: Date = Date()
    ) -> [ActivityGroup] {
        ...
        for activity in sortedActivities {
            if currentGroup.isEmpty {
                ...
                let distance = distanceBetween(
                    lastActivity, and: activity, width: width, selectedDate: selectedDate)
    ...
    private static func distanceBetween(
        _ a: Activity, and b: Activity, width: CGFloat, selectedDate: Date
    ) -> CGFloat {
        guard let timeA = a.scheduledTimeFor(date: selectedDate),
            let timeB = b.scheduledTimeFor(date: selectedDate)
        else {
        ...
        let posA = TimelinePositionHelper.positionForTime(
            hour: hourA, minute: minuteA, width: width)

        let hourB = Calendar.current.component(.hour, from: timeB)
        let minuteB = Calendar.current.component(.minute, from: timeB)
        let posB = TimelinePositionHelper.positionForTime(
            hour: hourB, minute: minuteB, width: width)
